...
class TblMapper {
    ...
    public function getCompiledSelect($reset = true) {
        ...
        return $this->rodb->get_compiled_select($this->tableName, $reset);
    }
    ...
    public function replace($set = null) {
        $this->compileSet()->resetWrite();
        $ret = $this->rwdb->replace($this->table, $set);
        $this->lastQuery = $this->rwdb->last_query();
    ...
    public function getCompiledUpdate($reset = true) {
        ...
        return get_compiled_update($this->tableName, $reset);
    }
    ...
    public function getCompiledDelete($reset = true) {
        ...
        return $this->rwdb->get_compiled_delete($this->tableName, $reset);
    }
    ...
    protected function compileSelect() {
        ...
        if (!empty($this->_having)) {
            foreach ($this->_having as $having) {
                switch ($having['type']) {
                    case 'having':
                        $this->rodb->having($having['key'], $having['escape']);
                        break;
                    case 'orHaving':
                        $this->rodb->orHaving($having['key'], $having['escape']);
                        break;
                }
            }
        }
        ...
        if (!empty($this->_orderBy)) {
            foreach ($this->_orderBy as $orderBy) {
                $this->rodb->group_by($orderBy['orderby'], $orderBy['direction'], $orderBy['escape']);
            }
        }
